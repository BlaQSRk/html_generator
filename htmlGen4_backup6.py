...
# def createTagsBackup(thisTag, htmlObjectList):
#     # Returns the created string.
#     if debug:
#         print("createTags()")
#     anchorAlignment = ''
...
#         # ParseContent does recursive calls to fill in the data. It's genius work, but I have no idea how it works
#         # Find Attributes turns the Attribute Dictionary list into a string for ease of use
#         return (createStartTag(thisTag) + findAttributes(thisTag) + endTag + parseContent(thisTag, htmlObjectList) +
#                 anchorAlignment + createStartTag(thisTag,True) + endTag)
#     else:
#         # SelfClosing tags have no content nor anchors
#         return (createStartTag(thisTag) + findAttributes(thisTag) + endTag)
...
def create_html_object(htmlObjectList, name, parentID = 'html', attributeList = [], content =''):
    ...
    add_tag_obj_to_list(htmlObjectList, thisTag) # Add the tag object to the list of HTML Tag Objects
